...
/**
 * <P>The object used for executing a static SQL statement                       // 用于执行静态 SQL 语句并返回它所生成结果的对象
 * and returning the results it produces.
 * <P>
 * By default, only one <code>ResultSet</code> object per <code>Statement</code> // 在默认情况下，同一时间每个 Statement 对象在只能打开一个 ResultSet 对象。
 * object can be open at the same time. Therefore, if the reading of one         // 因此，如果读取一个 ResultSet 对象与读取另一个交叉，则这两个对象必须是由不同的 Statement 对象生成的。
 * <code>ResultSet</code> object is interleaved
 * with the reading of another, each must have been generated by
 * different <code>Statement</code> objects. All execution methods in the        // 如果某个语句打开了一个 ResultSet 对象，则 Statement 接口中的所有执行方法都会隐式关闭它。例：com.mysql.cj.jdbc.StatementImpl#closeAllOpenResults()
